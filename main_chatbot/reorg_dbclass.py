...
def import_dataset():
    ...
    question_test = pd.read_csv('question_test.csv')
...
def train():
    ...
    checkpointer = ModelCheckpoint (filepath='dbsentence.h5', verbose=1, save_best_only=True)
...
def get_inference(query, embedding_matrix, tokenizer, MAX_LENGTH):
    model = get_multicnn_model(embedding_matrix)
    model.load_weights('dbsentence.h5')
    # Preparing the query
    in_query = []
